...
export async function deleteNonPendingSubmissions() {
  const res = await fetch(`${API_BASE}/submissions/delete-non-pending`, {
    method: "POST",
    credentials: "include",
  });
  return handleResponse(res);
...
export async function updateExchangePrice(id: number, priceUsdt: string) {
  const res = await fetch(`${API_BASE}/exchanges/${id}/price`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export async function toggleExchange(id: number) {
  const res = await fetch(`${API_BASE}/exchanges/${id}/toggle`, {
    method: "PATCH",
    credentials: "include",
  });
  return handleResponse(res);
...
export async function toggleUserEnabled(id: number) {
  const res = await fetch(`${API_BASE}/users/${id}/toggle-enabled`, {
    method: "PATCH",
    credentials: "include",
  });
  return handleResponse(res);
